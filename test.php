...
if($_GET[ans] == $cor)
{
	...
	$sql = "insert into `test_$username` (master_id, english, correct, time ) values ( '".$cor."','".$core."', '1',now() )";
	$result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
	echo "此單字累計正確";
	$sql="SELECT COUNT(*) AS C1 FROM `test_$username` WHERE `master_id`=$cor AND `correct`= '1'";
	$result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
	$row=$result -> fetch_array();
	echo $row[0];
	echo "次，錯誤";
	$sql="SELECT COUNT(*) AS C2 FROM `test_$username` WHERE `master_id`=$cor AND `correct`= '0'";
	$result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
	$row=$result -> fetch_array();
	echo $row[0];
	echo "次。";
...
else if($_GET[ans] != $cor)
{
	...
	$sql = "insert into `test_$username` (master_id, english, correct, time ) values ( '".$cor."','".$core."', '0',now() )";
	$result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
	echo "此單字累計正確";
	$sql="SELECT COUNT(*) AS C1 FROM `test_$username` WHERE `master_id`=$cor AND `correct`= '1'";
	$result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
...
$sql="select * from `test_$username` where `Correct`='1' group by `english`";
